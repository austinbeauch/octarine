...
from src.daomop import astrom, mpc
from src.planning import parameters
from src.validate.gui import config
...
class TrackTarget(TracksParser):
    ...
    def query_ssos(self, target_name, lunation_count=None):
        ...
        # so be generous with the search boundaries, add extra 2 weeks
        # current date just has to be the night of the triplet,
        from src.planning import horizons
...
class SSOSParser(object):
    ...
    def build_source_reading(expnum, ccd=None, ftype='p'):
        ...
        logger.debug("Building source reading for dataset_name:{} ccd:{} ftype:{}".format(expnum, ccd, ftype))
    ...
    def parse(self, ssos_result_filename_or_lines, mpc_observations=None):
        ...
        for row in ssos_table:
            ...
            if (row['Filter'] not in parameters.FILTERS) or row['Image_target'].startswith('WP'):
                continue
...
class Query(object):
    ...
    Inputs:
        - a list of daomop.mpc.Observation instances
